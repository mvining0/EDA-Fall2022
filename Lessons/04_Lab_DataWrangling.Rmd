...
NTL.phys.data.Tues1990.1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))
...
NTL.phys.data.ex4 <- NTL.phys.data.ex3 %>% filter(depth==0) %>% drop_na(temperature_C) %>% 
...
NTL.phys.data.ex5.gathered <- NTL.phys.data %>% pivot_longer(irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "radiation")
