...
Rainfall_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% #creating column for month in this action
...
BRA.complete <- BRA_complete %>%
  drop_na(Rainfall, Temperature_C) #none
...
plot.6 <- 
  ggplot(BRA.complete, aes(y = Temperature_C, x = Date),  size = 0.25) +
  geom_smooth(method = "lm") +
  geom_line()
print(plot.6)

plot.7 <- 
  ggplot(BRA.complete, aes(y = Rainfall, x = Date),  size = 0.25) +
  geom_smooth(method = "lm") +
  geom_line()
print(plot.7)
...
f_month <- month(first(BRA.complete$Date))
f_year <- year(first(BRA.complete$Date))

BRA_rainfall_ts <- ts(BRA.complete$Rainfall,
                   start=c(f_year,f_month,
                   frequency=12)) 
BRA_temp_ts <- ts(BRA.complete$Temperature_C,
                   start=c(f_year,f_month,
                   frequency=12)) 
...
rainfall_data_decomp <- stl(BRA_rainfall_ts,s.window = "periodic")
plot(rainfall_data_decomp)

temp_data_decomp <- stl(BRA_temp_ts,s.window = "periodic")
...
BRA_Rain_Components <-
  as.data.frame(rainfall_data_decomp$time.series[,1:3])
BRA_temp_Components <- 
  as.data.frame(temp_data_decomp$time.series[,1:3])
