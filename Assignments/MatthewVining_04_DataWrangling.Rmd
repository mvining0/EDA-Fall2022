...
Ozone.2018$Date <- as.Date(Ozone.2018$Date, format = "%Y/%m/%d")
class(Ozone.2018$Date)
...
#Ozone.2019$Date <- as.Date(Ozone.2019$Date, format = "%Y/%m/%d")
class(Ozone.2019$Date)
...
#PM25.2018$Date <- as.Date(PM25.2018$Date, format = "%Y/%m/%d")
class(PM25.2018$Date)
...
#PM25.2019$Date <- as.Date(PM25.2019$Date, format = "%Y/%m/%d")
class(PM25.2019$Date)
...
#9
#EPAair.8.summaries.spread <- pivot_wider(EPAair.8.summaries, names_from = AQS_PARAMETER_DESC, values_from = DAILY_AQI_VALUE)
...
> Answer: drop_na is a part of the tidyverse package that we installed at the beginning so we should feel more inclined to use a funciton of the packages we used for previous problems. Additionally, na.omit requires you create a new dataset to do so, but drop_na removes rows of NA from an existing dataset.
